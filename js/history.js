...
async function fetchUserSubmissions(userId) {
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      if (submissionData.isDeleted === true || submissionData.isActive === false) {
...
function updateStats(data) {
  ...
  const activeData = data.filter(s => s.isActive !== false && s.isDeleted !== true);

  const totalSubmissions = activeData.length;

  // Calculate total practices from metadata (more accurate)
  const totalPractices = activeData.reduce((sum, submission) => sum + (submission.totalPractices || 0), 0);
...
function calculateCurrentStreak(data) {
  ...
  for (let i = 0; i < sortedData.length; i++) {
    ...
    if (i === 0) {
      ...
      if (daysDiff > 1) { // If it's not today or yesterday
        break; // Gap too large, no current streak
      } else if (daysDiff === 1 && new Date(today.getFullYear(), today.getMonth(), today.getDate() - 1).getTime() !== submissionDate.getTime()) {
...
function calculateLongestStreak(data) {
  ...
  if (sortedData.length > 0) { // Ensure there's at least one submission
    maxStreak = 1;
  } else {
  ...
  for (let i = 0; i < sortedData.length - 1; i++) {
    ...
    } else if (diffDays > 1) { // Gap found, reset streak
      currentStreak = 1;
    }
 ...
 */
async function calculateDailyTotals(userId, dailySubmissionId, firestoreDb = db, realtimeDbRef = realtimeDb) {
    let totalPoints = 0;
    ...
    const categoriesSnapshot = await firestoreDb
        ...
        .collection('practice_submissions').doc(dailySubmissionId)
        .collection('categories')
        .get();

    categoriesSnapshot.forEach(catDoc => {
        const categoryData = catDoc.data();
        if (categoryData.isDeleted !== true) { // Only count active categories
            totalPoints += categoryData.totalPoints || 0;
 ...
 */
async function calculateUserTotals(userId, firestoreDb = db, realtimeDbRef = realtimeDb) {
    let totalPoints = 0;
    let totalPractices = 0;
    ...
    submissionsSnapshot.forEach(dailySubDoc => {
        ...
        if (dailySubmissionData.dailySubmissionId && dailySubmissionData.isDeleted !== true && dailySubmissionData.isActive !== false) {
...
function updatePracticeInLocalData(submissionId, categoryId, practiceIndex, updatedPractice) {
  ...
  submission.totalCategories = submission.categories.filter(cat => cat.isDeleted !== true).length; // Only count active categories

  // Update category summary for timeline display
  submission.categorySummary = submission.categories.filter(cat => cat.isDeleted !== true).map(cat => cat.name).join(', ');
...
function updateTimelineItem(submissionId) {
  ...
  const submission = allSubmissions.find(s => s.id === submissionId);
  if (!submission || submission.isDeleted === true || submission.isActive === false) {
  ...
  } else if (submission.submittedAt) { // Handle cases where it might just be a timestamp number
  ...
  const activeCategories = submission.categories.filter(cat => cat.isDeleted !== true); // Only active categories for display
...
window.saveEditedPractice = async function() {
  ...
  const selectedCategoryDisplayName = document.getElementById('editPracticeCategory').value; // Get the selected category display name
  ...
  try {
    const submissionRef = db.collection("users").doc(user.uid).collection("practice_submissions").doc(currentEditingSubmissionId);
    const categoryRef = submissionRef.collection("categories").doc(currentEditingCategoryId);
    ...
    const oldPractice = practices[currentEditingPracticeId];
    const oldPracticePoints = oldPractice.isDoublePoints ? (oldPractice.points || 0) * 2 : (oldPractice.points || 0);

    // Determine if double points applies to the new practice based on its category and date
    const today = new Date(currentViewingSubmission.date); // Use the original submission date
    today.setHours(0, 0, 0, 0);
    const isDoublePoints = shouldDoublePoints(selectedCategoryDisplayName, today); // Pass the category and date

    const updatedPractice = {
      ...oldPractice, // Keep existing fields
      name: selectedPracticeName,
      description: selectedDescription, // Update description as well
      points: selectedPoints, // Raw points
      isDoublePoints: isDoublePoints,
      lastModified: firebase.firestore.Timestamp.now()
    };

    // Add to edit history if significant changes occurred
    const changesMade = oldPractice.name !== updatedPractice.name || oldPractice.description !== updatedPractice.description || oldPractice.points !== updatedPractice.points || oldPractice.isDoublePoints !== updatedPractice.isDoublePoints;

    if (changesMade) {
        if (!updatedPractice.editHistory) {
            updatedPractice.editHistory = [];
        }
        updatedPractice.editHistory.push({
            editedAt: Date.now(),
            previousData: {
                name: oldPractice.name,
                description: oldPractice.description,
                points: oldPractice.points,
                isDoublePoints: oldPractice.isDoublePoints
            },
            changes: {
                name: updatedPractice.name,
                description: updatedPractice.description,
                points: updatedPractice.points,
                isDoublePoints: updatedPractice.isDoublePoints
            }
        });
        updatedPractice.isEdited = true;
    }


    // Update the specific practice in the array
    practices[currentEditingPracticeId] = updatedPractice;

    // Recalculate category totals (using potentially double points)
    const newCategoryTotalPoints = practices.reduce((sum, p) =>
      sum + (p.isDoublePoints ? (p.points || 0) * 2 : (p.points || 0)), 0
    );

    // Update both Firestore and Realtime Database for the category
    await Promise.all([
      categoryRef.update({
        practices: practices,
        ...
        lastModified: firebase.firestore.Timestamp.now(),
        categoryDisplayName: selectedCategoryDisplayName // Update category display name if changed
      }),
      realtimeDb.ref(`users/${user.uid}/practice_submissions/${currentEditingSubmissionId}/categories/${currentEditingCategoryId}`).update({
        ...
        categoryDisplayName: selectedCategoryDisplayName
      })
    ]);

    // Recalculate and update daily submission metadata and user totals
    // Get the previous daily total *before* the update
    const prevDailySubmissionDoc = await submissionRef.get();
    const prevDailyTotalPoints = prevDailySubmissionDoc.exists ? (prevDailySubmissionDoc.data().totalPoints || 0) : 0;

    const { totalPoints: newDailyTotalPoints, totalPractices: newDailyTotalPractices, totalCategories: newDailyTotalCategories } = await calculateDailyTotals(user.uid, currentEditingSubmissionId);
    const { totalPoints: newUserTotalPoints, totalPractices: newUserTotalPractices } = await calculateUserTotals(user.uid);
    ...
    await submissionRef.update({
      ...
      userTotalPoints: newUserTotalPoints, // Update at daily level
      userTotalPractices: newUserTotalPractices, // Update at daily level
      lastModified: firebase.firestore.Timestamp.now()
    });

    // Update daily submission metadata in Realtime Database
    await realtimeDb.ref(`users/${user.uid}/practice_submissions/${currentEditingSubmissionId}`).update({
      totalCategories: newDailyTotalCategories,
      totalPractices: newDailyTotalPractices,
      totalPoints: newDailyTotalPoints,
    ...
    // CRITICAL: Update totalUserScore at the user root
    const pointsDifference = newDailyTotalPoints - prevDailyTotalPoints;
    if (pointsDifference !== 0) {
        await updateRootUserTotals(user.uid, pointsDifference, 0, db, realtimeDb); // Only update score here, practices are cumulative from daily submissions
    }

    // Update local data and refresh UI
    updatePracticeInLocalData(currentEditingSubmissionId, currentEditingCategoryId, currentEditingPracticeId, updatedPractice);
    updateTimelineItem(currentEditingSubmissionId); // Update only the specific timeline item
    showSubmissionDetailsModal(allSubmissions.find(s => s.id === currentEditingSubmissionId)); // Refresh the modal content
    updateStats(allSubmissions); // Re-calculate and display overall stats
    ...
    showNotification('Error updating practice. Please try again. ' + error.message, 'error');
...
// ───────────────────────────────────────────────────────────
async function updateRootUserTotals(userId, pointsDifference, practicesDifference, firestoreDb, realtimeDb) {
    try {
        if (pointsDifference === 0 && practicesDifference === 0) {
            console.log("No difference in points or practices, skipping root user totals update.");
            return;
        }

        console.log(`Updating root user totals for ${userId}: pointsDiff=${pointsDifference}, practicesDiff=${practicesDifference}`);
        ...
        const currentRealtimeUserData = currentRealtimeUserSnapshot.val() || { totalUserScore: 0, totalUserPractices: 0 };

        const newTotalUserScore = (currentRealtimeUserData.totalUserScore || 0) + pointsDifference;
        const newTotalUserPractices = (currentRealtimeUserData.totalUserPractices || 0) + practicesDifference;
        ...
        console.log(`Root user totals updated successfully. New Score: ${newTotalUserScore}, New Practices: ${newTotalUserPractices}`);
    } catch (error) {
...
window.viewSubmissionDetails = function(submissionId) { // Removed dailySubmissionId as it's the same as submissionId
  let submission = allSubmissions.find(s => s.id === submissionId);

  if (!submission) {
    showNotification('Submission not found in local data. Re-fetching...', 'warning');
    // If not found locally, try to re-fetch and then show
    auth.currentUser && fetchUserSubmissions(auth.currentUser.uid).then(() => {
        ...
        if (submission) {
            ...
            showNotification('Submission still not found after re-fetch.', 'error');
        }
    });
    return;
  }
...
function showSubmissionDetailsModal(submission) {
  ...
  submission.categories.forEach(category => {
    ...
    category.practices.forEach((practice, index) => {
      ...
      const practiceName = practice.name || practice.practiceDescription || 'Unnamed Practice';
      const practiceDesc = practice.description || practice.originalPracticeData?.description || practice.practiceDescription || 'No description'; // Prefer description from original data if available
      const practicePoints = practice.isDoublePoints ? (practice.points || 0) * 2 : (practice.points || 0);
      const originalPoints = practice.points || 0; // The base points before doubling
      const isEditedIndicator = practice.isEdited ? '<span class="edited-indicator" title="This practice was manually edited."><i class="fas fa-pencil-alt"></i></span>' : '';
      const isDoublePointsIndicator = practice.isDoublePoints ? '<span class="double-points-indicator" title="Double Points Applied!"><i class="fas fa-star"></i></span>' : '';


      practicesHTML += `
        <div class="practice-item" data-practice-index="${index}" data-practice-id="${practice.id || ''}">
          <div class="practice-info">
            <h5>${practiceName} ${isEditedIndicator} ${isDoublePointsIndicator}</h5>
            <p>${practiceDesc}</p>
            <div class="practice-meta">
              <span class="points-display">Points: ${practicePoints}</span>
              ${practice.addedAt ? `<span>Added: ${new Date(practice.addedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>` : ''}
            </div>
          </div>
          <div class="practice-actions">
            ...
            <button class="delete-practice-btn" onclick="confirmDeletePractice('${submission.id}', '${category.id}', ${index}, '${practiceName}', '${submission.date}', ${originalPoints})" title="Delete Practice">
      ...
      <div class="category-section" data-category-id="${category.id}">
        <div class="category-header">
          ...
          <div class="category-actions">
              <button class="delete-category-btn" onclick="confirmDeleteCategory('${submission.id}', '${category.id}', '${category.name}', '${submission.date}')" title="Delete Category"><i class="fas fa-trash-alt"></i></button>
          </div>
        </div>
        <div class="practices-list">
          ${practicesHTML || '<p class="no-practices-message">No practices in this category.</p>'}
        </div>
      </div>
    `;
  });
    ...
    <div class="modal-container large-modal">
      ...
      <div class="modal-body">
        ...
        <div class="categories-container">
          ${categoriesHTML || '<div class="empty-state-message">No categories found for this submission.</div>'}
        </div>
      </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn-danger" onclick="confirmDeleteDailySubmission('${submission.id}', '${submission.date}', ${submission.totalPoints || 0}, ${submission.totalPractices || 0})">
...
function populateSubmissionsTimeline(data, filterType = 'all', startDate = null, endDate = null) {
  ...
  let filteredData = data.filter(submission => submission.isDeleted !== true && submission.isActive !== false);

  if (filterType !== 'all') {
    filteredData = filteredData.filter(submission =>
      submission.categories.some(cat => cat.name === filterType && cat.isDeleted !== true)
    );
  }
...
function applyFilters() {
  ...
  populateSubmissionsTimeline(allSubmissions, selectedCategory, startDate, endDate);
}
...
window.showConfirmationModal = function(title, message, onConfirmCallback, confirmButtonText = 'Confirm', confirmButtonClass = 'btn-danger') {
...
window.confirmDeletePractice = function(submissionId, categoryId, practiceIndex, practiceName, submissionDate, originalPoints) {
    const title = 'Delete Individual Practice?';
    const message = `Are you sure you want to delete "${practiceName}" (${originalPoints} pts) from your session on ${new Date(submissionDate).toLocaleDateString()}? This action cannot be undone.`;
    showConfirmationModal(title, message, async () => {
        await deletePracticeItem(auth.currentUser.uid, submissionId, categoryId, practiceIndex, originalPoints, submissionDate);
        ...
        if (currentViewingSubmission && currentViewingSubmission.id === submissionId) {
...
window.confirmDeleteCategory = function(submissionId, categoryId, categoryName, submissionDate) {
    const title = 'Delete Category Submission?';
    const message = `Are you sure you want to delete the entire "${categoryName}" category from your session on ${new Date(submissionDate).toLocaleDateString()}? This will remove all practices within it. This action cannot be undone.`;
    showConfirmationModal(title, message, async () => {
        await deleteCategorySubmission(auth.currentUser.uid, submissionId, categoryId, submissionDate);
...
window.confirmDeleteDailySubmission = function(submissionId, submissionDate, totalPoints, totalPractices) {
    const title = 'Delete Entire Daily Session?';
    const message = `Are you sure you want to delete your entire practice session from ${new Date(submissionDate).toLocaleDateString()} (Total Points: ${totalPoints}, Total Practices: ${totalPractices})? This will remove all associated data. This action cannot be undone.`;
    showConfirmationModal(title, message, async () => {
        await deleteDailySubmission(auth.currentUser.uid, submissionId, submissionDate, totalPoints, totalPractices);
        showNotification(`Daily session from ${new Date(submissionDate).toLocaleDateString()} deleted.`, 'success');
 ...
 */
async function deletePracticeItem(userId, dailySubmissionId, categoryDocId, practiceIndex, originalPoints, submissionDate) {
    ...
    try {
        const submissionRef = db.collection('users').doc(userId).collection('practice_submissions').doc(dailySubmissionId);
        const categoryRef = submissionRef.collection('categories').doc(categoryDocId);

        // 1. Get the current category document
        const categoryDoc = await categoryRef.get();
        if (!categoryDoc.exists) {
            console.warn("Category document not found for deletion:", categoryDocId);
        ...
        const isDoublePointsPeriod = shouldDoublePoints(categoryData.categoryDisplayName, new Date(submissionDate));
        ...
        const isAllowMultiple = practicesData[categoryData.categoryDisplayName]?.find(p => p.name === practiceName)?.allowMultiple;
        ...
        if (removedPractices.length === 0) {
            console.warn("Practice not found at index, already removed or invalid operation.");
            return;
        }
        ...
        if (newCategoryTotalPractices === 0) {
            ...
            realtimePromises.push(realtimeDb.ref(`users/${userId}/practice_submissions/${dailySubmissionId}/categories/${categoryDocId}`).update({
                isDeleted: true,
                totalPoints: 0,
                totalPractices: 0,
        ...
        const { totalPoints: newDailyTotalPoints, totalPractices: newDailyTotalPractices, totalCategories: newDailyTotalCategories } = await calculateDailyTotals(userId, dailySubmissionId);

        // Get the previous daily total from the metadata document before updating it
        const prevDailySubmissionDoc = await submissionRef.get();
        const prevDailyTotalPoints = prevDailySubmissionDoc.exists ? (prevDailySubmissionDoc.data().totalPoints || 0) : 0;
        const prevDailyTotalPractices = prevDailySubmissionDoc.exists ? (prevDailySubmissionDoc.data().totalPractices || 0) : 0;
        ...
        // Update daily submission metadata in Realtime Database
        await realtimeDb.ref(`users/${userId}/practice_submissions/${dailySubmissionId}`).update({
            totalCategories: newDailyTotalCategories,
            totalPractices: newDailyTotalPractices,
            totalPoints: newDailyTotalPoints,
            lastModified: realtimeTimestamp
        ...
        const pointsDifferenceForRoot = newDailyTotalPoints - prevDailyTotalPoints;
        const practicesDifferenceForRoot = newDailyTotalPractices - prevDailyTotalPractices;

        await updateRootUserTotals(userId, pointsDifferenceForRoot, practicesDifferenceForRoot, db, realtimeDb);
        ...
        if (practiceName && !isAllowMultiple) {
            ...
            const dailyCompletionsRef = db.collection("users").doc(userId).collection("daily_completions").doc(todayDateString);
            await dailyCompletionsRef.update({
                completed_practices: firebase.firestore.FieldValue.arrayRemove(practiceName)
            });
            console.log(`Removed "${practiceName}" from daily completions for ${todayDateString}`);
        }

        console.log('Practice item deleted successfully and all totals recalculated!');
 ...
 * @param {string} submissionDate The date string of the daily submission (YYYY-MM-DD)
 */
async function deleteCategorySubmission(userId, dailySubmissionId, categoryDocId, submissionDate) {
    const firestoreTimestamp = firebase.firestore.FieldValue.serverTimestamp();
    const realtimeTimestamp = firebase.database.ServerValue.TIMESTAMP;
    ...
    try {
        ...
        await realtimeDb.ref(`users/${userId}/practice_submissions/${dailySubmissionId}/categories/${categoryDocId}`).update({
            isDeleted: true,
            totalPoints: 0,
            totalPractices: 0,
            deletedAt: realtimeTimestamp,
        ...
        const singleSubmissionPractices = practicesInDeletedCategory.filter(p => !p.originalPracticeData?.allowMultiple); // Filter based on original data
        if (singleSubmissionPractices.length > 0) {
            await dailyCompletionsRef.update({
                completed_practices: firebase.firestore.FieldValue.arrayRemove(...singleSubmissionPractices.map(p => p.name))
            });
            console.log(`Removed associated single-entry practices from daily completions for ${todayDateString}`);
        }

        console.log('Category submission deleted successfully and all totals recalculated!');
        await refreshAllUIComponents(userId);
 ...
 */
async function deleteDailySubmission(userId, dailySubmissionId, submissionDate, totalPointsToDelete, totalPracticesToDelete) {
    const firestoreTimestamp = firebase.firestore.FieldValue.serverTimestamp();
    const realtimeTimestamp = firebase.database.ServerValue.TIMESTAMP;
    ...
    try {
        ...
        const categoriesSnapshot = await submissionRef.collection('categories').get();
        ...
        categoriesSnapshot.forEach(catDoc => {
            const categoryData = catDoc.data();
            if (categoryData.isDeleted !== true) { // Only process active categories
                ...
                realtimeCategoryPromises.push(realtimeDb.ref(catDoc.ref.path).update({
                    isDeleted: true,
                    totalPoints: 0,
                    totalPractices: 0,
                    deletedAt: realtimeTimestamp,
                ...
                (categoryData.practices || []).forEach(p => {
                    const originalPracticeDef = practicesData[categoryData.categoryDisplayName]?.find(pd => pd.name === p.name);
        ...
        console.log(`All categories for daily submission ${dailySubmissionId} soft-deleted.`);
        ...
        await updateRootUserTotals(userId, -totalPointsToDelete, -totalPracticesToDelete, db, realtimeDb);

        // 4. Remove associated practices from daily_completions
        if (practicesToRemoveFromDailyCompletions.length > 0) {
            const dailyCompletionsRef = db.collection("users").doc(userId).collection("daily_completions").doc(submissionDate);
            await dailyCompletionsRef.update({
                completed_practices: firebase.firestore.FieldValue.arrayRemove(...practicesToRemoveFromDailyCompletions)
            });
            console.log(`Removed associated single-entry practices from daily completions for ${submissionDate}`);
        }

        console.log('Daily submission deleted successfully and all totals recalculated!');
        await refreshAllUIComponents(userId);
...
// ───────────────────────────────────────────────────────────
    ...
    function showExportModal(data) {
      ...
      if (data.length > 0) {
        const activeData = data.filter(s => s.isDeleted !== true && s.isActive !== false); // Only export active data
    ...
    function updateExportPreview(data) {
      ...
      let filteredData = data.filter(s => s.isDeleted !== true && s.isActive !== false); // Always filter out deleted/inactive
    ...
    window.executeExport = function() {
      ...
      let exportData = allSubmissions.filter(s => s.isDeleted !== true && s.isActive !== false);
      if (startDate) {
    ...
    function convertToCSV(data, includeCategories, includePractices, includeStats) {
      ...
      const rows = data.map(submission => {
        ...
        if (includeCategories) {
          row.push(submission.categories.filter(c => c.isDeleted !== true).map(cat => `${cat.name} (${cat.totalPractices} practices, ${cat.totalPoints} pts)`).join('; '));
        }

        if (includePractices) {
          const practiceDetails = submission.categories.filter(c => c.isDeleted !== true).map(cat =>
            `${cat.name}: ${cat.practices.map(p => `${p.name} (${p.isDoublePoints ? (p.points || 0) * 2 : (p.points || 0)} pts)`).join(', ')}`
          ).join(' | ');
          row.push(practiceDetails);
        }

        if (includeStats) {
          row.push(submission.submittedAt && submission.submittedAt.toDate ? submission.submittedAt.toDate().toLocaleString() : 'N/A');
        }
    ...
    function convertToJSON(data, includeCategories, includePractices, includeStats) {
      const exportData = data.map(submission => {
        ...
        if (includeCategories) {
          item.categoriesSummary = submission.categories.filter(c => c.isDeleted !== true).map(cat => ({
            name: cat.name,
        ...
        if (includePractices) {
          item.individualPractices = submission.categories.filter(c => c.isDeleted !== true).map(cat => ({
            category: cat.name,
        ...
        if (includeStats) {
          item.submittedAt = submission.submittedAt && submission.submittedAt.toDate ? submission.submittedAt.toDate().toISOString() : undefined;
          item.lastModified = submission.lastModified && submission.lastModified.toDate ? submission.lastModified.toDate().toISOString() : undefined;
        }
    ...
    function showPrintPreview(data) {
      ...
      const printContent = generatePrintContent(data.filter(s => s.isDeleted !== true && s.isActive !== false)); // Only print active data
    ...
    function generatePrintContent(data) {
      ...
      const submissionsHTML = data.map(submission => {
        ...
        const categoriesHTML = submission.categories.filter(c => c.isDeleted !== true).map(cat => `
          <div class="print-category">
            <strong>${cat.name}</strong> - ${cat.totalPractices} practices, ${cat.totalPoints} points
            <ul>
              ${cat.practices.map(p => `<li>${p.name} (${p.isDoublePoints ? (p.points || 0) * 2 : (p.points || 0)} pts)</li>`).join('')}
            </ul>
          </div>
        `).join('');
