document.addEventListener("DOMContentLoaded", () => {
    ...
    usersRef.on(
      ...
      (snapshot) => {
        try {
          if (!snapshot.exists()) {
            elements.leaderboardContainer.innerHTML =
              "<div class='no-data'>No leaderboard data available yet.</div>";
            return;
          }
  ...
  function processLeaderboardData(usersData) {
    ...
    Object.keys(usersData).forEach((userId) => {
      ...
      if (userData.practice_submissions) {
        Object.values(userData.practice_submissions).forEach(
          (dailySubmission) => {
              ...
              Object.values(dailySubmission.categories).forEach((category) => {
                if (!category.isDeleted) {
                  ...
                  if (
                    ...
                    category.submittedAt.timestamp > latestSubmissionTime
                  ) {
  ...
  function createLeaderboardRow(rank, entry) {
    ...
    row.innerHTML = `
        ...
        <div class="name">
          <button class="player-name-btn" data-userid="${
            entry.userId
          }" data-username="${escapeHtml(entry.name)}">
  ...
  function createPlayerHistoryModal() {
    ...
    document
      .getElementById("playerHistoryModalCloseBtn")
        ...
        for (const date in groupedByDate) {
          historyHtml += `
                        <div class="timeline-day-group">
                            <h4 class="timeline-date-heading">${new Date(
                              date
          ...
          groupedByDate[date].forEach((p) => {
            historyHtml += `
                            <li class="practice-log-item">
                                ...
                                <span class="log-name">${escapeHtml(
                                  p.name
                                )}</span>
                                <span class="log-points">${p.points} pts</span>
                                ${
                                  p.isEdited
                                    ? '<span class="log-edited" title="This practice was edited.">Edited</span>'
                                    : ""
                                }
                            </li>`;
          });
