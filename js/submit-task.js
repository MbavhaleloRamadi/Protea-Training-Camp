document.addEventListener("DOMContentLoaded", () => {
  ...
  firebase.auth().onAuthStateChanged((user) => {
    ...
    const dailyCompletionsRef = firebase
      .firestore()
      .collection("users")
      .doc(userId)
    ...
    dailyCompletionsRef
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          console.log(
            "Fetched today's completed practices:",
  ...
  const specialPointsPeriods = {
    ...
    Fitness: [{ start: createDate(2025, 8, 25), end: createDate(2025, 8, 31) }],
  };
  ...
  if (practiceContainer && practiceContainer.parentNode) {
    practiceContainer.parentNode.insertBefore(
      specialPointsInfo,
      practiceContainer
    );
  }
  ...
  const practicesData = {
    Putting: [
      ...
      },
      {
        name: "Putt-Under36",
        description:
          "Practice under 36 putts (2 putt average) for 18 different holes longer than 3 metres",
        points: 15,
      },
      {
    ...
    Mental: [
      ...
      {
        name: "Mind Soduko",
        description: "Complete a game of Sudoku",
        points: 3,
      },
      {
        name: "Mind Reflect",
      ...
      {
        name: "Mind Control",
        description: "Exercise full deep breathing exercises for 30mins",
        points: 10,
      },
      {
        name: "Mind Learn",
    ...
    "On The Course": [
      ...
      {
        name: "OTC-Partime",
        description: "Score a Par on a Hole (unlimited per day)",
        points: 5,
        allowMultiple: true,
      },
      {
        name: "OTC-Par3",
        description: "Score a par or lower on a par 3 (unlimited per day)",
        points: 5,
        allowMultiple: true,
      },
      {
        name: "OTC-Up&Down",
        description:
          "Score an Up&Down for par or lower out of a greenside bunker (unlimited per day)",
        points: 5,
        allowMultiple: true,
      },
      {
      ...
      {
        name: "OTC-Birdies",
        description: "Score a Birdie on a Hole (unlimited per day)",
        points: 10,
        allowMultiple: true,
      },
      {
      ...
      {
        name: "OTC-Eagle",
        description: "Score an Eagle (unlimited per day)",
        points: 25,
        allowMultiple: true,
      },
    ],
  ...
  function updateSelectedPracticesDisplay() {
    ...
    if (!selectedList || !selectedLabel) {
      console.error(
        "Required elements for selected practices display not found"
      );
      return;
    }
    ...
    selectedPractices.forEach((practice, index) => {
      ...
      <div class="practice-points-section">
        <span class="practice-points ${
          practice.isDoublePoints ? "bonus-points" : ""
        }">
    ...
    taskCategory.addEventListener("change", () => {
      ...
      practicesData[cat].forEach((practice) => {
        ...
        if (
          ...
          todaysCompletedPractices.includes(practice.name)
        ) {
        ...
        border-left: 4px solid ${
          practice.allowMultiple ? "#28a745" : "#6c757d"
        };
        width: 100%;
        max-width: 600px;
      ...
      `;
        ...
        const displayPoints = isSpecialPointsDay
          ? practice.points * 2
          : practice.points;
        ...
        <div style="display: flex; flex-direction: column; width: 100%; text-align: left; height: 100%;">
          <h4 style="margin-top: 0; margin-bottom: 8px; font-size: 1rem; color: #222222;">${
            practice.name
          }</h4>
          <p style="margin: 0 0 10px; font-size: 0.9rem; color: #4a4a4a; line-height: 1.4;">${
      ...
      `;
        ...
        card.addEventListener("click", () => {
          ...
          if (
            !practice.allowMultiple &&
            selectedPractices.find((p) => p.name === practice.name)
          ) {
          ...
          if (practice.allowMultiple) {
            ...
            const count = selectedPractices.filter(
          ...
          showConfirmation(
            `✅ Added "${practice.name}"${
              practice.allowMultiple
                ? ` (${
                    selectedPractices.filter((p) => p.name === practice.name)
                      .length
                  })`
                : ""
            }`,
  ...
  async function updateTotalUserScore(userId, pointsDifference) {
    try {
      ...
      console.log(
        `Updating totalUserScore by ${pointsDifference} points for user ${userId}`
      );

      // Update Firestore using increment
      const userDocRef = dbFirestore.collection("users").doc(userId);
      await userDocRef.update({
        "practice_submissions.totalUserScore":
          firebase.firestore.FieldValue.increment(pointsDifference),
      });

      // Update Realtime Database
      const userScoreRef = dbRealtime.ref(
        `users/${userId}/practice_submissions/totalUserScore`
      );
      ...
      console.log(
        `Successfully updated totalUserScore: ${currentScore} + ${pointsDifference} = ${newScore}`
      );
    } catch (error) {
  ...
  function updateSelectedPracticesDisplayWithContainer(displayContainer) {
    ...
    `;
      ...
      console.log(
        `Updated display: ${selectedPractices.length} practices, ${totalPoints} total points`
      );
    } catch (error) {
  ...
  function addPracticeToSelection(practice) {
    try {
      ...
      const existingIndex = selectedPractices.findIndex(
        (p) => p.name === practice.name || p.id === practice.id
      );
      ...
      // Show confirmation
      showConfirmation(
        `Added: ${practice.name} (${practice.points} pts)`,
        "success"
      );
  ...
  if (submitForm) {
    ...
    submitForm.addEventListener("submit", async (event) => {
      ...
      if (!areSubmissionsAllowed(today)) {
        showConfirmation(
          "Submissions are not allowed during tournament days.",
          "red"
        );
        return;
      }
      ...
      try {
        ...
        const previousDailyPoints = await getCurrentDailyPoints(
          userId,
          dailySubmissionId
        );
        ...
        const submissionBasketId = `${currentDate}_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`;

        // Group practices by category
        const practicesByCategory = selectedPractices.reduce(
          (acc, practice, index) => {
            const categoryKey = practice.category.toLowerCase();
            if (!acc[categoryKey]) {
              acc[categoryKey] = {
                categoryDisplayName:
                  practice.categoryDisplay || practice.category,
                practices: [],
              };
            }
        ...
        for (const [categoryKey, categoryData] of Object.entries(
          ...
          const practicesArray = categoryData.practices.map(
            (practice, index) => {
              ...
              if (!fullPracticeData) {
                for (const [catName, practices] of Object.entries(
                  practicesData
                )) {
                  fullPracticeData = practices.find(
                    (p) => p.name === practice.name || p.name === practice.id
                  );
          ...
          const firestoreSubmissionData = {
            ...
            totalPoints: practicesArray.reduce(
              (sum, p) => sum + (p.isDoublePoints ? p.points * 2 : p.points),
              0
            ),
          ...
          const realtimeRef = dbRealtime.ref(
        ...
        const singleSubmissionPractices = selectedPractices
          .filter(
            (p) =>
              !practicesData[p.category].find((pd) => pd.name === p.name)
                ?.allowMultiple
          )
          .map((p) => p.name);
        ...
        if (singleSubmissionPractices.length > 0) {
          ...
          await dailyCompletionsRef.set(
            {
              completed_practices: firebase.firestore.FieldValue.arrayUnion(
                ...singleSubmissionPractices
              ),
            },
            { merge: true }
        ...
        const { dailyTotalPoints, dailyTotalPractices, dailyTotalCategories } =
          await calculateDailyTotals(userId, dailySubmissionId);
        const {
          totalPoints: userTotalPoints,
          totalPractices: userTotalPractices,
        ...
        console.log(
          `Points difference: ${dailyTotalPoints} - ${previousDailyPoints} = ${pointsDifference}`
        );
        ...
        if (existingMetadata.exists) {
          ...
          const realtimeMetadataRef = dbRealtime.ref(
        ...
        console.log("🏆 User Lifetime Totals:", {
          userTotalPoints,
          userTotalPractices,
        });
        ...
        showConfirmation(
          "Failed to submit practices. Please try again.",
          "red"
        );
      } finally {
  ...
  async function editPracticeItem(
    userId,
    dailySubmissionId,
    categoryDocId,
    practiceId,
    ...
    try {
      ...
      const categoryTotalPoints = updatedPractices.reduce(
        (sum, p) => sum + (p.isDoublePoints ? p.points * 2 : p.points),
        0
      );
...
async function deletePracticeItem(
  ...
  try {
    ...
    const { dailyTotalPoints, dailyTotalPractices, dailyTotalCategories } =
      await calculateDailyTotals(userId, dailySubmissionId);
    const { totalPoints: userTotalPoints, totalPractices: userTotalPractices } =
      await calculateUserTotals(userId);
...
async function deleteCategorySubmission(
  ...
  try {
    ...
    console.log(
      "Category submission deleted successfully and totals recalculated"
    );
    return true;
  } catch (error) {
...
// ───────────────────────────────────────────────────────────
async function addPracticeToCategory(
  userId,
  dailySubmissionId,
  categoryDocId,
  newPractice
  ...
  try {
    ...
    const practiceEntry = {
      ...
      practiceItem:
        newPractice.id || newPractice.name.toLowerCase().replace(/\s+/g, "_"),
    ...
    console.log(
      "Practice added to category successfully and totals recalculated"
    );
    return true;
  } catch (error) {
